...
2. We then enrich the set of variables. Bill has become extremely interested in graph embedding, and enriches the dataset with variables that are the position vectors for a variety of graph embeddings. Chentian has...**(add what you would like to do/have done Chentian :-)**.

3. After our exotic variable enrichment we re-train our random forest on the new dataset.

We will assess whether our enrichment methods 'learnt' anything worthwhile about the data in two ways. The first is qualitative: did the performance of our classsifier change significantly? The second is quantitative: we look at variable importance metrics for our random forest, and see if any of our constructed variables score highly.
